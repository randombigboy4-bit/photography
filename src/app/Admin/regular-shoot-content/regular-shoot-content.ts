...
export class RegularShootContent implements OnInit {
  ...
  constructor(private api: ApiServices, private fb: FormBuilder, private toastr: ToastrService) {}
  ...
  loadPackages() {
    this.api.getPackagesByEventName('Regular Pre-Wedding').subscribe({
      next: (res: any) => {
  ...
  edit(item: any, type: string) {
    ...
    fileInput.onchange = (e: any) => {
      ...
      this.api.updateRegular(item.eventId, formData).subscribe({
        next: () => {
  ...
  deleteItem(item: any, type: string) {
    ...
    this.api.deleteRegular(item.eventId, {
      type,
      index: item.index,
    }).subscribe({
      next: () => {
